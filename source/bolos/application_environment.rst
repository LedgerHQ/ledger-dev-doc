...
Due to its limited amount of RAM, the Secure Element is designed to only support
...
:doc:`isolates the application </background/application_isolation>` from the
other applications on the device, and restricts access to all areas of flash
memory other than those exclusively allocated for the running application.

This model has the tremendous advantage of not limiting what the application can
do, however it also implies that every application has to do **all** of the
...
option to customize I/O protocols for more specialized applications.

.. figure:: images/app_centric_view.png
   :alt: Application-centric view
   :align: center

   The application-centric view of the BOLOS environment

The above diagram shows a view of the system as seen by the application. The app
...
Some peripherals not only receive commands from the SE, but also trigger events
...
In this model, the application is at the center, and does not rely on any other
embedded co-applications.

Delegation Model
----------------

.. figure:: images/usb_delegation_overview.png
   :alt: USB delegation overview
   :align: center

   An overview of the USB delegation model

Once BOLOS boots the application, BOLOS is not reachable anymore, it only
...
peripherals (like USB) and therefore BOLOS does not play a role in I/O handling.

Featuring these two key points, applications are in charge on the device. This
