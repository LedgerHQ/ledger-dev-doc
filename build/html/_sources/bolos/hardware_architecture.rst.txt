...
Ledger devices have a very unique architecture in order to leverage the security
...
the hardware related constraints before analyzing their software implications.

Multiple Processors: Secure Element Proxy
-----------------------------------------

.. figure:: images/bolos_architecture.png
   :alt: Detailed BOLOS architecture
   :align: center

   A detailed BOLOS architecture diagram

BOLOS is split between two hardware chips, one being secure (the ST31 Secure
Element), and the other having JTAG enabled and acting as a proxy (the STM32
MCU).

Furthermore, the Secure Element is also split into two parts: the firmware which
...
the protocol between the SE and the MCU is done by the currently running app).

BOLOS relies on the collaboration of both chips to empower Secure Element
...
The SEPROXYHAL protocol is structured as a serialized list of three types of
...
implemented to avoid such cases.

The protocol works as follows:

1. The MCU sends an Event (button press, ticker, USB transfer, ...).
2. The SE responds with a list of zero or more Commands in response to the
   Event.
3. The SE sends a Status indicating that the Event is fully processed and waits
   for another Event.

.. figure:: images/seproxyhal.png
   :alt: SEPROXYHAL protocol concept
   :align: center

   SEPROXYHAL protocol concept

As a matter of fact, due to buffer size, requests to display something to the
...
implemented in the SDK, which will be discussed further in the next chapter.

The SE throws an exception to applications willing to send more than one Status
in a row, without a new Event being fetched in between.
